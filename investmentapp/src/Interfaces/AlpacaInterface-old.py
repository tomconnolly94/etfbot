...
class AlpacaInterface(InvestingInterface):
    ...
    def getStockDataList(self: object, stockSymbols: "list[str]") -> "list[StockData]":
        ...
        return [
            StockData(symbol, snapshot.latest_trade.p)
            for symbol, snapshot in stockSnapShots.items()
        ]
    ...
    def getLastYearPortfolioPerformance(self):
        outputDict = {}
        try:
            data: PortfolioHistory = self.api.get_portfolio_history(
    ...
    def _submitOrder(self, stockSymbol, quantity, order) -> None:
        if self.devMode:
            return
        self.api.submit_order(symbol=stockSymbol, qty=quantity, side=order)
