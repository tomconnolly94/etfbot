...
class LinearWeightingStrategy(StockChoiceStrategy):
    ...
    def getBuyOrders(
        self: object, availableFunds: int, existingPositions: "dict[str, int]"
    ) -> "dict[str, int]":
        stockDataList = self._getStockRangeIdeal()
        if self._reverseStockDataList:
            stockDataList.reverse()
        stockDataList = self._reorderStockDataListBasedOnExistingPositions(
    ...
    def getPositionInStockDataList(
        self, stockSymbol: str, stockDataList: list[StockData]
    ):
    ...
    def _reorderStockDataListBasedOnExistingPositions(
        self, stockDataList: "list[StockData]", existingPositions: "dict[str, int]"
    ):
