...
from webapp.server.controllers.DataServer import (
    ...
    _normaliseValues,
    _getSPY500Data,
    _getCurrentHoldingsPerformanceData,
    _getPortfolioPerformanceData,
)
...
class Test_DataServer(unittest.TestCase):
    ...
    def test__normaliseValues(self):
        ...
        # config fake data
        fakeData = {"key1": 1, "key2": 2, "key3": 3, "key4": 4, "key5": 5}
    ...
    def test__getSPY500Data(self, getPricesForStockSymbolsMock, _normaliseValuesMock):
        ...
        expectedStockData = {
            "currentValue": fakeStockData[0][
                364
            ],  # sum([value[364] for value in fakeStockData ]),
            "oneMonthPrevValue": fakeStockData[0][
                334
            ],  # sum([value[334] for value in fakeStockData ]),
            "oneYearPrevValue": fakeStockData[0][
                0
            ],  # sum([value[0] for value in fakeStockData ]),
        ...
        self.assertEqual(
            expectedStockData["oneMonthPrevValue"], stockData["oneMonthPrevValue"]
        )
        self.assertEqual(
            expectedStockData["oneYearPrevValue"], stockData["oneYearPrevValue"]
        )

    @mock.patch("webapp.server.controllers.DataServer._normaliseValues")
    @mock.patch("webapp.server.controllers.DataServer.getPricesForStockSymbols")
    @mock.patch("webapp.server.controllers.DataServer.AlpacaInterface")
    def test__getCurrentHoldingsPerformanceData(
        self, AlpacaInterfaceMock, getPricesForStockSymbolsMock, _normaliseValuesMock
    ):
        ...
        for positionIndex, positionHistory in enumerate(historicalPositionPrices):
            positionHistory[364] = openPositions[
                list(openPositions.keys())[positionIndex]
            ]
    ...
    def test__getPortfolioPerformanceData(
        self, DatabaseInterfaceMock, getPricesForStockSymbolsMock, _normaliseValuesMock
    ):
        ...
        date_list = [
            (base - datetime.timedelta(days=x)).date() for x in range(364, -1, -1)
        ]
        ...
        DatabaseInterfaceMagicMock.getPortfolioValueOverTime.return_value = (
            fakePortfolioHistory
        )
